    ...
    <section dir="rtl" className={["grid gap-5 sm:grid-cols-2 xl:grid-cols-4", className].filter(Boolean).join(" ")}>
      {TOOLS.map((tool) => (
        <Link
          key={tool.slug}
          href={tool.href}
          className="group relative flex flex-col overflow-hidden rounded-[var(--corner-radius)] border-[length:var(--border-size)] border-[var(--border-color)] bg-[var(--card-background)] p-5 shadow-[var(--shadow-size)] transition-all duration-[200ms] hover:-translate-y-1 hover:shadow-[var(--shadow-size)]"
        >
          <div
