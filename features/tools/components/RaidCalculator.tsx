...
export default function RaidCalculator(){
 ...
 <h3 className=" text-[length:var(--font-size-h2)] text-[var(--h2-font-color)] font-bold text-[var(--raid)]">RAID Calculator</h3>
 <span className="text-[length:var(--font-size-paragraph)] text-[var(--paragraph-color)] text-muted-foreground">تکباکس – ابزار زیرساخت</span>
 </div>

 <div className="grid sm:grid-cols-3 gap-3 text-[length:var(--font-size-h3)] text-[var(--h3-font-color)] font-semibold">
 <label className="space-y-1">
 <span className="text-[length:var(--font-size-paragraph)] text-[var(--paragraph-color)] text-muted-foreground">تعداد دیسک</span>
 <input type="number" min={2} max={24} value={disks} onChange={e=>setDisks(Math.max(2, parseInt(e.target.value)||2))} className="input" />
 </label>
 <label className="space-y-1">
 <span className="text-[length:var(--font-size-paragraph)] text-[var(--paragraph-color)] text-muted-foreground">ظرفیت هر دیسک (GB)</span>
 <input type="number" min={100} step={100} value={size} onChange={e=>setSize(parseInt(e.target.value)||0)} className="input" />
 </label>
 <label className="space-y-1">
 <span className="text-[length:var(--font-size-paragraph)] text-[var(--paragraph-color)] text-muted-foreground">سطح RAID</span>
 ...
 <div className="text-[length:var(--font-size-paragraph)] text-[var(--paragraph-color)] text-muted-foreground">{k}</div>
