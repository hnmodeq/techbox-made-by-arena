...
export default function RaidCalculator(){
 ...
 <div className="grid sm:grid-cols-3 gap-3 text-[length:var(--h3-font-size)] text-[var(--h3-font-color)] font-semibold">
 <label className="space-y-1">
 <span className="text-[length:var(--paragraph-font-size)] text-[var(--paragraph-color)] text-muted-foreground">تعداد دیسک</span>
 <input type="number" min={2} max={24} value={disks} onChange={e=>setDisks(Math.max(2, parseInt(e.target.value)||2))} className="input" />
 </label>
 <label className="space-y-1">
 <span className="text-[length:var(--paragraph-font-size)] text-[var(--paragraph-color)] text-muted-foreground">ظرفیت هر دیسک (GB)</span>
 <input type="number" min={100} step={100} value={size} onChange={e=>setSize(parseInt(e.target.value)||0)} className="input" />
 </label>
 <label className="space-y-1">
 <span className="text-[length:var(--paragraph-font-size)] text-[var(--paragraph-color)] text-muted-foreground">سطح RAID</span>
 ...
 <div key={k as string} className="rounded-[var(--corner-radius)] border-[length:var(--border-size)] border-[var(--border-color)] bg-[var(--card-background)]/60 p-3">
