  ...
  return (
    <div className="space-y-5" dir="rtl">
      ...
      <div className="card p-5 space-y-2">
        <div className="flex items-center gap-2 text-[length:var(--font-size-h2)] text-[var(--h2-font-color)] font-bold text-[var(--subnet)]">
          <Icon name="server" size={20} strokeWidth={1.75} />
          ماشین‌حساب ساب‌نت (Subnet) چیست؟
        </div>
        <p className="text-[length:var(--font-size-h3)] text-[var(--h3-font-color)] font-semibold paragraph-color leading-8">
      ...
      <div className="card p-5 space-y-4">
        <div className="grid sm:grid-cols-2 gap-4">
          <label className="space-y-1 block">
            <span className="text-[length:var(--font-size-paragraph)] text-[var(--paragraph-color)] paragraph-color">آدرس IP</span>
            <input
              value={ip}
            ...
            />
            {!valid && <span className="text-[length:var(--font-size-paragraph)] text-[var(--paragraph-color)] text-[var(--tb-danger)]">آدرس IP معتبر نیست (مثال: 192.168.1.0)</span>}
          </label>
          <label className="space-y-2 block">
            ...
            />
            <div className="flex justify-between text-[length:var(--font-size-paragraph)] text-[var(--paragraph-color)] paragraph-color" dir="ltr">
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
          {(["network", "broadcast", "mask", "first", "last", "hosts"] as const).map((k) => (
            <div key={k} className="rounded-[var(--corner-radius)] border border-[var(--border-color)] bg-[var(--card-background)] px-3 py-2.5">
              <div className="text-[length:var(--font-size-paragraph)] text-[var(--paragraph-color)] paragraph-color">{RESULT_LABELS[k]}</div>
