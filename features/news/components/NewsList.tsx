...
export default function NewsList() {
 ...
 {items.map((n: any) => (
                <Link key={n.slug} href={`/news/${n.slug}`} className="card overflow-hidden group hover:shadow-[var(--tb-shadow-lg)] transition-all !p-0 grid sm:grid-cols-3 gap-4 items-center">
                  <div className="block relative aspect-[16/9] sm:aspect-[4/3] sm:h-full overflow-hidden bg-[var(--tb-bg-muted)]">
                    ...
                    <span className="absolute top-3 right-3 rounded-[var(--tb-radius-full)] border border-white/30 bg-transparent px-2 py-1 text-[length:var(--paragraph-font-size)] text-[var(--paragraph-color)] text-white backdrop-blur-[var(--tb-blur-sm)]">{n.category}</span>
                  </div>
                  <div className="p-4 sm:col-span-2">
                    <div className="text-[length:var(--paragraph-font-size)] text-[var(--paragraph-color)] flex flex-wrap items-center gap-2 text-[var(--tb-fg-muted)]">
                    ...
                    <h3 className="text-[length:var(--h2-font-size)] font-bold text-[var(--h2-font-color)] mt-2 transition-colors group-hover:text-[var(--tb-news)]">{n.title}</h3>
                    <p className="text-[length:var(--paragraph-font-size)] text-[var(--paragraph-color)] line-clamp-2 mt-2 text-[var(--tb-fg-muted)]">{n.excerpt}</p>
 ...
 <h3 className="text-[length:var(--h3-font-size)] font-semibold text-[var(--h3-font-color)] text-[var(--tb-news)]">اخبار فوری</h3>
 <span className="inline-flex items-center gap-1.5 rounded-[var(--tb-radius-full)] border border-[var(--tb-border)] px-2 py-0.5 text-[length:var(--paragraph-font-size)] text-[var(--paragraph-color)] text-[var(--tb-fg-muted)]">
                ...
                {forceNews.map((f: any) => (
                  <li key={f.slug} className="relative pr-7">
                    ...
                    <Link href={`/news/${f.slug}`} className="group block">
                      <div className="text-[length:var(--paragraph-font-size)] text-[var(--paragraph-color)] flex items-center gap-1 text-[var(--tb-fg-muted)]">
