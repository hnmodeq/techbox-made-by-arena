...
export default function NewsTicker({ items, className = "" }: NewsTickerProps) {
 ...
 className={`ticker-item group flex shrink-0 items-center gap-2 whitespace-nowrap text-[length:var(--font-size-h3)] text-[var(--h3-font-color)] font-semibold paragraph-color transition-colors duration-[var(--tb-motion-sm)] ${hoverTone}`}
 >
 ...
 {when && <span className="text-[length:var(--font-size-paragraph)] text-[var(--paragraph-color)] paragraph-color">• {when}</span>}
 </Link>
 );
 })}
 </div>
