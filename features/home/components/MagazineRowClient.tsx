...
export default function MagazineRowClient({ articles }: { articles: ContentItem[] }) {
  return (
          ...
          {articles.map((art) => (
            ...
            >
              ...
              <div className="px-4 pb-4 pt-3 border-t-[length:var(--border-size)] border-[var(--border-color)]/60 flex items-center justify-between text-xs paragraph-color font-bold">
                ...
                <CardStats module={art.module || 'blog'} slug={art.slug} initialViews={art.views ?? 0} initialLikes={art.likes ?? 0} showComments={true} />
              </div>
            </div>
          ))}
        </div>
