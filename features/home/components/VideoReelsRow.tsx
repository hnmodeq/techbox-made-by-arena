    ...
    <section className={`w-full py-8 px-4 sm:px-6 lg:px-8 bg-[var(--main-background)] ${HOME_ROW_SIZES.mediaMinHeight} flex flex-col justify-center`} dir="rtl">
      <div className={`mx-auto ${HOME_ROW_SIZES.containerMaxWidth} w-full`}>
        <div className="flex items-center justify-between gap-4 mb-6">
          <h2 className="text-xl sm:text-2xl font-black text-[var(--primary-text)]">{homeTitle || "آخرین ویدیوهای کوتاه تکباکسی"}</h2>
          <Link href="/media" className="text-sm font-bold text-[var(--media)] hover:underline flex items-center gap-1 shrink-0"><span>{homeMoreLabel || "گشت و گزار در ویدیوها"}</span><span>←</span></Link>
        </div>
        {loading ? (
