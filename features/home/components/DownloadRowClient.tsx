...
function DownloadMeta({ slug, initialViews, initialLikes }: { slug: string; initialViews: number; initialLikes: number }) {
  ...
  useEffect(() => {
    ...
    fetch(`/api/stats?module=download&slug=${encodeURIComponent(slug)}`)
      .then(r => r.json())
      .then(d => { if (mounted && d && typeof d.fileSize === 'string') setFileSize(d.fileSize); })
      .catch(() => {});
  ...
  return (
    <div className="flex flex-wrap items-center justify-between gap-2 w-full mt-3">
      ...
      <div className="flex items-center gap-3">
        ...
        <CardStats module="download" slug={slug} initialViews={initialViews} initialLikes={initialLikes} showComments={true} />
      </div>
    </div>
  );
}
...
export default function DownloadRowClient({ files }: { files: ContentItem[] }) {
  return (
      ...
      <div className={`mx-auto ${HOME_ROW_SIZES.containerMaxWidth} w-full`}>
        ...
        <div className="grid gap-4 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4">
          {files.map((file) => {
            ...
            return (
              <Link key={file.slug} href={`/download/${file.slug}`} className="group bg-[var(--card-background)] text-[var(--primary-text)] border-[length:var(--border-size)] border-[var(--border-color)] rounded-[var(--corner-radius)] shadow-[var(--shadow-size)] p-4 hover:bg-[var(--muted-background)]/40 transition-all duration-[200ms] border-[length:var(--border-size)] border-[var(--border-color)] flex flex-col justify-between gap-3 bg-[var(--card-background)]">
                <div className="flex items-start gap-3.5 min-w-0">
                  ...
                  <div className="min-w-0 flex-1">
                    <h3 className="text-[length:var(--h3-font-size)] text-[var(--h3-font-color)] font-semibold font-bold text-[var(--primary-text)] group-hover:text-[var(--download)] transition-colors line-clamp-1 leading-6">{file.title}</h3>
                    <div className="text-xs paragraph-color mt-1 line-clamp-1">{file.excerpt || 'سرورهای Enterprise و مجازی‌سازی'}</div>
                  </div>
                </div>
                <DownloadMeta slug={file.slug} initialViews={file.views ?? 0} initialLikes={file.likes ?? 0} />
              </Link>
            );
          })}
        </div>
