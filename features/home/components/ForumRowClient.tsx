...
export default function ForumRowClient({ topics }: { topics: ContentItem[] }) {
  return (
      ...
      <div className={`mx-auto ${HOME_ROW_SIZES.containerMaxWidth} w-full`}>
        ...
        <div className="grid gap-4 sm:grid-cols-2 md:grid-cols-3">
          {topics.map((top) => (
            ...
            >
              <Image src={top.author?.avatar || '/assets/hooman.png'} alt={top.author?.name || 'کاربر'} width={48} height={48} className="h-12 w-12 rounded-full object-cover ring-1 ring-[var(--border-color)] shrink-0" />
              <div className="flex-1 min-w-0">
                <div className="flex items-center justify-between gap-2 mb-1.5 flex-wrap">
                  <span className="text-[length:var(--paragraph-font-size)] text-[var(--paragraph-color)] font-bold paragraph-color">{top.author?.name || 'عضو تکباکس'}</span>
                  <ForumBadge slug={top.slug} />
                </div>
                <h3 className="text-[length:var(--h3-font-size)] text-[var(--h3-font-color)] font-semibold font-bold text-[var(--primary-text)] group-hover:text-[var(--forum)] transition-colors line-clamp-2 leading-6">{top.title}</h3>
