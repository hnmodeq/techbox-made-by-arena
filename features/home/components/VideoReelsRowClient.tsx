...
export default function VideoReelsRowClient({ videos }: { videos: ContentItem[] }) {
          ...
          {videos.map((vid) => (
            ...
            >
              ...
              <div className="absolute inset-0 flex items-center justify-center z-20 pointer-events-none">
                <div className="w-12 h-12 rounded-full bg-white/25 backdrop-blur-md flex items-center justify-center text-white transition-transform group-hover:scale-125 shadow-[var(--shadow-size)]">▶</div>
              </div>
              <div className="relative z-30 p-3.5 text-white w-full">
                ...
                <div className="mt-2.5">
                  <CardStats module="media" slug={vid.slug} initialViews={vid.views ?? 0} initialLikes={vid.likes ?? 0} showComments={true} />
                </div>
              </div>
            </button>
          ))}
          ...
          <div className="fixed inset-0 flex items-center justify-center p-3 sm:p-6" style={{ zIndex: zIndex.modal }} dir="rtl">
            ...
            <div className="relative w-full max-w-4xl max-h-[90vh] overflow-y-auto rounded-[var(--corner-radius)] border-[length:var(--border-size)] border-[var(--border-color)] bg-[var(--modal-background)] shadow-[var(--shadow-size)] flex flex-col" style={{ zIndex: zIndex.modalContent }}>
              <div className="flex items-center justify-between p-4 border-b-[length:var(--border-size)] border-[var(--border-color)]">
                ...
                <button type="button" onClick={() => setActiveVideo(null)} className="inline-flex h-8 w-8 items-center justify-center rounded-full bg-[var(--muted-background)] paragraph-color hover:text-[var(--primary-text)] transition-colors cursor-pointer" aria-label="بستن"><Icon name="close" size={18} /></button>
              </div>
              <div className="relative aspect-video bg-black shrink-0">
                <video key={activeVideo.slug} controls autoPlay playsInline poster={activeVideo.image} className="w-full h-full object-contain bg-black" src={SAMPLE_VIDEO} />
              </div>
