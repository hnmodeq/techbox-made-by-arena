...
export default function ShopRowClient({ products }: { products: ContentItem[] }) {
  return (
          ...
          {products.map((prod) => (
            ...
            >
              <div className="relative aspect-[4/3] w-full overflow-hidden bg-[var(--muted-background)]/70 p-4 flex items-center justify-center">
                <Image src={prod.image || '/assets/blog-1.jpg'} alt={prod.title} fill className="object-cover" sizes="(max-width: 768px) 100vw, 300px" />
              </div>
              <div className="p-4 flex-1 flex flex-col justify-between">
                <div>
                  <h3 className="text-[length:var(--h3-font-size)] text-[var(--h3-font-color)] font-semibold font-black text-[var(--primary-text)] group-hover:text-[var(--shop)] transition-colors line-clamp-2 min-h-[44px] leading-6">{prod.title}</h3>
                  <p className="mt-2.5 text-xs paragraph-color leading-5 line-clamp-3">{prod.excerpt}</p>
                </div>
                <div className="mt-4 space-y-3">
                  <div className="flex justify-start items-center">
                    <CardStats module="shop" slug={prod.slug} initialViews={prod.views ?? 0} initialLikes={prod.likes ?? 0} showComments={true} />
                  </div>
                  <span className="text-xs font-black text-[var(--shop)] hover:underline flex items-center justify-center py-1">ثبت درخواست خرید ←</span>
                </div>
              </div>
            </Link>
          ))}
