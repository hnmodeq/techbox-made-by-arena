    ...
    <main className="mx-auto max-w-6xl px-4 py-10" dir="rtl">
      <ModuleHeader module="forum" title="انجمن تکباکس" description="پرسش و پاسخ تخصصی زیرساخت و شبکه" count={`${all.length.toLocaleString("fa-IR")} موضوع`}>
        <div className="flex gap-2">
          <input placeholder="جستجو در انجمن…" className="input w-56 text-[length:var(--font-size-h3)] text-[var(--h3-font-color)] font-semibold" />
          <Button onClick={() => setShowNew(true)} className="text-[length:var(--font-size-h3)] text-[var(--h3-font-color)] font-semibold">+ موضوع جدید</Button>
        </div>
      </ModuleHeader>

      <div className="flex gap-2 text-[length:var(--font-size-paragraph)] text-[var(--paragraph-color)] mb-4">
      ...
      <div className="card divide-y divide-[var(--border-color)]/60 overflow-hidden">
        <div className="hidden sm:grid grid-cols-12 text-[length:var(--font-size-paragraph)] text-[var(--paragraph-color)] paragraph-color px-4 py-2.5 bg-[var(--muted-background)]/30 font-bold">
        ...
        {all.map((t) => (
          <Link key={t.slug} href={`/forum/${t.slug}`} className="group grid grid-cols-12 px-3 sm:px-4 py-3.5 hover:bg-[var(--muted-background)]/20 gap-2 items-center transition-colors">
            {/* vote column */}
            <div className="hidden sm:flex col-span-1 flex-col items-center paragraph-color text-[length:var(--font-size-paragraph)] text-[var(--paragraph-color)]">
            ...
            <div className="col-span-12 sm:col-span-6 flex gap-3.5 items-center">
              ...
              <div className="min-w-0 flex-1">
                <div className="flex items-center gap-2 flex-wrap">
                  <span className="text-[length:var(--font-size-h3)] text-[var(--h3-font-color)] font-semibold font-bold transition-colors group-hover:text-[var(--forum)]">{t.title}</span>
                  <ForumBadge slug={t.slug} fallback={t.solved} />
                </div>
                <div className="text-[length:var(--font-size-paragraph)] text-[var(--paragraph-color)] paragraph-color mt-1">
            ...
            <div className="hidden sm:block col-span-1 text-left text-[length:var(--font-size-paragraph)] text-[var(--paragraph-color)] paragraph-color">
        ...
        <div className="fixed inset-0 flex items-center justify-center p-4" style={{ zIndex: zIndex.modal }} dir="rtl">
          ...
          <form onSubmit={submitTopic} className="relative card w-full max-w-2xl p-6 space-y-4 z-10 shadow-2xl">
            <div className="flex justify-between items-center border-b border-[var(--border-color)] pb-3">
              <h3 className="text-[length:var(--font-size-h2)] text-[var(--h2-font-color)] font-bold font-bold">موضوع جدید در انجمن تکباکس</h3>
            ...
            <div className="text-[length:var(--font-size-paragraph)] text-[var(--paragraph-color)] paragraph-color">پیش‌نویس به‌صورت خودکار در مرورگر ذخیره می‌شود.</div>
