    ...
    <main className="mx-auto max-w-5xl px-4 py-12" dir="rtl">
      ...
      <div className="flex flex-col gap-6 mt-8">
        {items.map((r: any, i: number) => {
          ...
          return (
            <Link key={r.slug} href={`/review/${r.slug}`} className="card overflow-hidden group grid md:grid-cols-[280px_1fr] lg:grid-cols-[320px_1fr] gap-6 !p-0 transition-all hover:shadow-[var(--tb-shadow-lg)] items-stretch">
              <div className="block relative aspect-[16/10] md:aspect-auto md:h-full bg-[var(--tb-bg-muted)] overflow-hidden min-h-[220px]">
                <Image src={r.image || "/assets/blog-1.jpg"} fill sizes="(min-width:1024px) 320px, 100vw" className="object-cover transition-transform duration-[var(--tb-motion-lg)] group-hover:scale-105" alt={r.title} />
                <span className="absolute top-3 right-3 rounded-full border border-white/30 bg-black/50 px-3 py-1 text-xs font-bold text-white backdrop-blur-md">{r.category}</span>
              </div>
              ...
              <div className="p-5 sm:p-6 flex flex-col justify-between">
                <div>
                  ...
                  <p className="mt-3 text-[14px] leading-7 text-[var(--tb-fg-muted)] line-clamp-3">
                    {r.excerpt}
                  </p>
                </div>

                {/* Author & stats footer */}
                <div className="flex flex-wrap items-center justify-between gap-4 mt-6 pt-4 border-t border-[var(--tb-border)]">
