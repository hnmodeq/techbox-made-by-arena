...
export function ContentCard({ item, compact = false }: { item: ContentItem; compact?: boolean }) {
 ...
 <h4 className={`mt-1 line-clamp-2 text-[length:var(--font-size-paragraph)] text-[var(--paragraph-color)] text-[var(--primary-text)] transition-colors ${moduleHover(item.module)}`}>{item.title}</h4>
 {!compact && <p className="mt-1 line-clamp-2 text-[length:var(--font-size-paragraph)] text-[var(--paragraph-color)] paragraph-color">{item.excerpt}</p>}
          <div className="mt-2">
...
export function ContentFeedList({ items, variant="compact" }: { items: ContentItem[]; variant?: "compact"|"image"|"video"|"forum"|"product"|"download"|"review" }) {
 if (!items.length) return <div className="py-6 text-center text-[length:var(--font-size-paragraph)] text-[var(--paragraph-color)] paragraph-color">مطلبی نیست</div>;
...
function VideoFeedCard({item}:{item:ContentItem}){
 ...
 <div className="relative aspect-video overflow-hidden rounded-[var(--corner-radius)] bg-black">
      ...
      <div className="px-1 pt-2">
        <div className={`line-clamp-2 text-[length:var(--font-size-paragraph)] text-[var(--paragraph-color)] transition-colors ${moduleHover(item.module)}`}>{item.title}</div>
        <div className="mt-1">
          <CardStats module={item.module} slug={item.slug} initialViews={item.views ?? 0} initialLikes={item.likes ?? 0} showComments={true} />
        </div>
      </div>
...
function ForumFeedCard({item}:{item:ContentItem}){
 ...
 <div className="min-w-0 flex-1">
 <div className={`line-clamp-2 text-[length:var(--font-size-paragraph)] text-[var(--paragraph-color)] transition-colors ${moduleHover(item.module)}`}>{item.title}</div>
 <div className="mt-1 flex flex-wrap items-center gap-2 text-[length:var(--font-size-paragraph)] text-[var(--paragraph-color)] paragraph-color">
 <span>{item.author?.name || "کاربر"}</span>
 <span>• {answers} پاسخ</span>
            <span className={`rounded-[var(--corner-radius)] border px-1.5 py-0.5 text-[length:var(--font-size-paragraph)] text-[var(--paragraph-color)] ${solved ? "border-[color-mix(in_oklch,var(--tb-success)_45%,transparent)] text-[var(--tb-success)]" : "border-[color-mix(in_oklch,var(--tb-warning)_45%,transparent)] text-[var(--tb-warning)]"}`}>{solved ? "حل‌شده" : "باز"}</span>
 </div>
 </div>
 </Link>
 );
...
function ProductFeedCard({item}:{item:ContentItem}){
 ...
 <span className="absolute left-2 top-2 rounded-[var(--corner-radius)] border border-white/30 bg-transparent px-2 py-0.5 text-[length:var(--font-size-paragraph)] text-[var(--paragraph-color)] text-white backdrop-blur-[var(--tb-blur-sm)]">موجود</span>
 </div>
 <div className="px-1 pt-2">
 <div className={`line-clamp-2 min-h-[34px] text-[length:var(--font-size-paragraph)] text-[var(--paragraph-color)] transition-colors ${moduleHover(item.module)}`}>{item.title}</div>
 <div className="mt-1 text-[length:var(--font-size-paragraph)] text-[var(--paragraph-color)] text-[var(--shop)]">۴۸,۹۰۰,۰۰۰ <span className="text-[length:var(--font-size-paragraph)] text-[var(--paragraph-color)] paragraph-color">تومان</span></div>
 </div>
 </Link>
 );
}
...
function DownloadFeedCard({item}:{item:ContentItem}){
 ...
 <div className="min-w-0 flex-1">
 <div className={`line-clamp-1 text-[length:var(--font-size-paragraph)] text-[var(--paragraph-color)] font-bold transition-colors ${moduleHover(item.module)}`}>{item.title}</div>
 <div className="mt-0.5 text-[length:var(--font-size-paragraph)] text-[var(--paragraph-color)] paragraph-color">{item.date_fa} • {item.category}</div>
 </div>
 <div className="text-[length:var(--font-size-paragraph)] text-[var(--paragraph-color)] paragraph-color shrink-0 font-bold">{(item.likes ?? 0).toLocaleString("fa-IR")} بار دانلود</div>
 </Link>
 );
}
...
function ReviewFeedCard({item}:{item:ContentItem}){
 ...
 <span className="text-[length:var(--font-size-paragraph)] text-[var(--paragraph-color)] paragraph-color">{item.author?.name || "تکباکس"}</span>
 </div>
          <div className="mt-1">
            <CardStats module={item.module} slug={item.slug} initialViews={item.views ?? 0} initialLikes={item.likes ?? 0} showComments={true} />
          </div>
