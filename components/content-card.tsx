...
export function ContentCard({ item, compact = false }: { item: ContentItem; compact?: boolean }) {
  ...
  return (
    <Link href={`/${item.module}/${item.slug}`} className="group block rounded-2xl border border-border bg-card/70 p-3 hover:bg-card hover:border-primary/30 transition-colors">
      <div className="flex gap-3">
        ...
        <div className="min-w-0 flex-1">
          ...
          <h4 className="text-[13px] font-bold leading-6 line-clamp-2 mt-1 group-hover:text-primary transition-colors">{item.title}</h4>
