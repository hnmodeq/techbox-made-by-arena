...
export function Avatar({ src, alt, size=40, className, ...p }: { src?: string; alt?: string; size?: number } & React.HTMLAttributes<HTMLSpanElement>){
 ...
 <span className="flex h-full w-full items-center justify-center text-[length:var(--font-size-paragraph)] text-[var(--paragraph-color)] paragraph-color">{(alt||"?").slice(0,2).toUpperCase()}</span>}
 </span>
 );
}
