...
export const MediaSelectorCard = React.forwardRef<HTMLButtonElement, MediaSelectorCardProps>(
 ...
 {category && (
 <span className="absolute top-3 right-3 rounded-full border border-white/30 bg-black/40 px-2.5 py-0.5 text-[length:var(--paragraph-font-size)] text-[var(--paragraph-color)] text-white backdrop-blur-md">
 {category}
 </span>
 )}
 <span className="absolute top-3 left-3 rounded-[var(--tb-radius-sm)] bg-black/60 px-2 py-0.5 text-[length:var(--paragraph-font-size)] text-[var(--paragraph-color)] text-white backdrop-blur-sm">
 {duration}
 </span>

 <div className="absolute bottom-0 inset-x-0 p-4 text-white z-10 flex flex-col justify-end">
 <h4 className="text-[length:var(--h3-font-size)] font-semibold text-[var(--h3-font-color)] font-bold line-clamp-2 transition-colors duration-[var(--tb-motion-sm)] group-hover:text-[var(--tb-media)]">
 {title}
 </h4>
