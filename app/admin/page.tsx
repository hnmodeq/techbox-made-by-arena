...
type ModuleStats = { module: ModuleSlug; count: number; views: number; latest?: string };
...
export default function AdminPage() {
  ...
  // Fetch stats from API instead of using sync getModuleItems
  useEffect(() => {
    if (!user) return;
    setLoadingStats(true);
    fetch("/api/stats")
      .then(r => r.json())
      .then((data: Record<string, { views: number }>) => {
        const modules = moduleOrder.filter(m => canEdit(user, m));
        const result: ModuleStats[] = modules.map(module => {
          const moduleStats = Object.entries(data)
            .filter(([key]) => key.startsWith(`${module}:`))
            .reduce((acc, [, val]) => ({
              count: acc.count + 1,
              views: acc.views + (val.views || 0),
            }), { count: 0, views: 0 });
          return {
            module,
            count: moduleStats.count,
            views: moduleStats.views,
            latest: "—",
          };
        });
        setStats(result);
      ...
      <section className="mx-auto max-w-6xl space-y-8">
        ...
        <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
          ...
          <div className="bg-[var(--card-background)] text-[var(--primary-text)] border-[length:var(--border-size)] border-[var(--border-color)] rounded-[var(--corner-radius)] shadow-[var(--shadow-size)] p-4">
            ...
            <div className="mt-1 text-[length:var(--h1-font-size)] text-[var(--h1-font-color)] font-extrabold ">{loadingStats ? "..." : totals.count.toLocaleString("fa-IR")}</div>
          </div>
          <div className="bg-[var(--card-background)] text-[var(--primary-text)] border-[length:var(--border-size)] border-[var(--border-color)] rounded-[var(--corner-radius)] shadow-[var(--shadow-size)] p-4">
            <div className="text-[length:var(--paragraph-font-size)] text-[var(--paragraph-color)] paragraph-color">بازدید منبع‌ها</div>
            <div className="mt-1 text-[length:var(--h1-font-size)] text-[var(--h1-font-color)] font-extrabold ">{loadingStats ? "..." : totals.views.toLocaleString("fa-IR")}</div>
          </div>
          <div className="bg-[var(--card-background)] text-[var(--primary-text)] border-[length:var(--border-size)] border-[var(--border-color)] rounded-[var(--corner-radius)] shadow-[var(--shadow-size)] p-4">
        ...
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {stats.map(({ module, count, views, latest }) => {
