...
export default function CheckoutPage(){
 ...
 <h1 className="text-[length:var(--h1-font-size)] text-[var(--h1-font-color)] font-extrabold mb-2 text-[var(--shop)]">تسویه حساب – زرین‌پال</h1>
 <p className="mb-6 text-[length:var(--paragraph-font-size)] text-[var(--paragraph-color)] paragraph-color">
 ...
 <label className="flex items-center gap-2 rounded-[var(--corner-radius)] border-[length:var(--border-size)] border-[var(--home)] bg-[color-mix(in_oklch,var(--home)_8%,transparent)] px-3 py-2">
 ...
 <label className="flex items-center gap-2 rounded-[var(--corner-radius)] border-[length:var(--border-size)] border-[var(--border-color)] px-3 py-2"><input type="radio" name="pay" disabled /> کارت به کارت (غیرفعال در دمو)</label>
 </div>

 <Button onClick={pay} disabled={loading || items.length===0} className="w-full text-[length:var(--h3-font-size)] text-[var(--h3-font-color)] font-semibold disabled:opacity-60">
 ...
 items.map(i=>(
 <div key={i.slug} className="flex justify-between border-b-[length:var(--border-size)] border-[var(--border-color)] pb-2">
 ...
 <div className="flex justify-between border-t-[length:var(--border-size)] border-[var(--border-color)] pt-2 text-[length:var(--h3-font-size)] text-[var(--h3-font-color)] font-semibold "><span>مبلغ قابل پرداخت</span><span className="text-[var(--shop)]">{totalToman.toLocaleString("fa-IR")} تومان</span></div>
 <div className="text-[length:var(--paragraph-font-size)] text-[var(--paragraph-color)] paragraph-color">≈ {(amountRial).toLocaleString("fa-IR")} ریال – درگاه زرین‌پال</div>
 </div>
 </div>
 </div>
 </main>
